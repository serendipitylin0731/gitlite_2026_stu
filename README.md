...
同时，我们为你添加了一个`main.cpp`、两个建议类`Commit`和`Repository`，以及`main.cpp`所依赖的命令门面类`SomeObj`（骨架中已给出与`main.cpp`匹配的方法签名，实现为空，需要你自行补全），以帮助入门。`main.cpp`及`SomeObj.h`中的公开方法签名是评测接口，请勿修改；你可以自由调整其余类的设计。你还需要自行编写`CMakeLists.txt`完成项目的编译与运行。为了评测正常运行，编译后生成的可执行文件必须位于`./build/gitlite`。
...
每个提交前都有一个`===` ，提交后有一个空行。与真正的 `Git` 一样，每个条目都显示提交对象的唯一 `SHA-1` ID。提交中显示的时间戳反映的是**当前时区**，而不是 UTC；因此，初始提交的时间戳不是 1970 年 1 月 1 日星期四 00:00:00，而是等效的北京时间。输出时请保证其按照`EEE MMM dd HH:mm:ss yyyy Z`的格式输出（其中`dd`表示日期按两位补零，如`01`），项与项之间用空格隔开；
...
- 与`log`只显示从当前头提交沿第一父提交形成的历史链不同，`global-log`要求显示对象库中的所有`commit`，甚至包括`reset`到某个旧提交后不再属于任何分支的提交。`commit`的顺序无关紧要。
...
在本子任务中，你可以选择完善`status`功能、完成`remote`功能（包括`add-remote`,`rm-remote`,`push`,`fetch`,`pull`），或者完成`diff`功能，也可以三者兼有。Bonus 提供 35 个原始分：`status`为 10 分，`remote`共 15 分，`diff`共 10 分。**Bonus 最终最多计入 25 分；原始得分超过 25 分时按 25 分计算。**
...
- 如果给定名称的远程已经存在，则打印错误消息：`A remote with that name already exists.`后退出。你不必检查用户名和服务器信息是否合法，也无需检查该路径是否存在——即使路径不存在，`add-remote`也应当静默成功；路径问题延迟到`fetch`/`push`/`pull`时才报告`Remote directory not found.`。
...
对于项目中的代码部分，我们给出每个`Subtask` 的分数占比，具体的分数由 OJ 的评测机给定：
